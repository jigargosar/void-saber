Proper ECS Modeling for void-saber2.ts
...
Replace the per-frame polling `createSaberSetupSystem` with a reactive pipeline using `bridgeQuery` + `entityVisualBridge` + `driveTransformer`:
...
function createVisualPipeline(): Teardown {
  ...
  const disposeDriver = driveTransformer(entities, (entity) => {
    const saber = toSaber(entity);      // memoized — creates once per entity
    const trail = toTrail(entity);      // memoized — creates once per entity
    if (!('saber' in entity)) {
  ...
  return () => { disposeDriver(); disposeBridge(); };
}
```

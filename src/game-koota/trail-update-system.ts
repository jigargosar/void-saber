...
import { Saber, TrailMesh, TrailData, GripBound } from './traits'
import {
...
export function createTrailUpdateSystem(getDelta: () => number): () => void {
  return () => {
    const dt = getDelta()

    for (const entity of world.query(Saber, TrailMesh, TrailData, GripBound)) {
      const buffers = entity.get(TrailData)
      const saber = entity.get(Saber)
      const mesh = entity.get(TrailMesh)

      if (!buffers || !saber || !mesh || !buffers.started) continue

      const bp = saber.blade.base.getAbsolutePosition()
      const tp = saber.blade.tip.getAbsolutePosition()
